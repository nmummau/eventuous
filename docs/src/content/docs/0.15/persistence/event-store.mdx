...
Eventuous has several implementations of event store abstraction, you can find them in the [infrastructure](../../infra/esdb) section. The default implementation is `EsdbEventStore`, which uses [EventStoreDB](https://eventstore.com) as the event store. It's a great product, and we're happy to provide first-class support for it. It's also a great product for learning about Event Sourcing and CQRS.
...
As Eventuous 0.15 stopped using the `IAggregateStore` abstraction, and all related APIs are marked obsolete.

Aggregate persistence is now supported by a few functions extending the `IEventReader` and `IEventWriter` interfaces, described on the [Aggregate store](../aggregate-store) page.

## State persistence

The event store interfaces are quite low-level, which allows supporting various underlying databases with relatively small amount of code. However, for higher-level persistence operations, this core API isn't very practical. This usability gap is covered by additional functions that extend code event store interfaces, providing easier to use API for application developers. Most of those functions are also used by Eventuous, particularly in command services.

### Storing state updates

Persisting state updates are done by appending events to the entity stream. The low-level API for that is the `AppendEvents` function. For convenience, the `IEventWriter` interface also provides a higher-level function that makes this easier.
